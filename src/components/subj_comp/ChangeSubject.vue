...
const subjectCollectionQuery = query(subjectCollectionRef, orderBy("date", "desc"));
//
...
<template>
    <div class="h-screen flex flex-col justify-center items-center">
        <div class="md:mr-100 mr-80">
            <button @click="storeCounter.showChangeSubj"  class="rounded p-1 hover:bg-white hover:bg-opacity-80">
        ...
        <div class=" bg-beigeL rounded-lg shadow-2xl w-96 md:w-200 h-3/4">
            ...
            <div>
                ...
                <div class="flex flex-col overflow-y-scroll  pt-3 h-110 mb-8 bg-white mx-5 rounded shadow ">
                    <!-- Iterate all subject -->
                    <div v-for="subj in subjects" class=" mx-3">
                        <div class="flex justify-center">
                            <button @click="[setGlobalSubjectID(subj.id), storeCounter.showChangeSubj()], NoteList.filterNotes" class="h-10 mb-3 w-170 mr-2 bg-beigeD rounded-3xl hover:bg-beigeSD">
                               {{ subj.subj_title }}
                            </button>
                            <button @click="deleteSubject(subj.id)" class="justify-end h-10 text-black font-bold hover:text-red-500">
                                &cross;
                            </button>    
                        </div>
                    </div>
