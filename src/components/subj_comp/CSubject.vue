...
<template>
  <div
    class="flex flex-col justify-center gap-4 md:grid md:grid-rows-3 md:gap-0 mx-5 mt-5 mb-3 h-40 bg-white rounded shadow">
    ...
    <form @submit.prevent="addSubject">
      <div class="flex px-3 md:px-0 md:justify-center -mt-1">
        <input
          v-model="newSubjTitle"
          class="h-10 w-170 p-3 rounded border-gray-200 border-1 shadow-sm focus:outline-none focus:border-beigeD focus:ring-beigeD focus:ring-1 focus:shadow-lg"
          type="text"
          placeholder="New Quiz Title" />
      </div>
      <div class="flex justify-center -mt-1">
        <button
          :disabled="!newSubjTitle"
          class="mt-4 font-semibold bg-beigeD h-10 w-52 rounded-lg shadow-md hover:bg-beigeSD hover:shadow-xl">
          Create Subject
        </button>
      </div>
    </form>
