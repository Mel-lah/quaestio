...
<template>
    <div class="h-screen flex flex-col justify-center items-center">
        <div class="md:mr-100 mr-80">
            <button @click="[storeCounter.showONote(), updateNote()]" class="rounded p-1 hover:bg-white hover:bg-opacity-80">
        ...
        <div class="bg-beigeL rounded-lg shadow-2xl w-96 md:w-200 h-3/4">    
            <form  @submit.prevent="addNote">
                <div class="m-5 flex justify-between">
                    <textarea v-model="noteTitle" wrap="hard" class="p-2 w-full text-lg font-se h-10 text-black rounded overflow-hidden overscroll-none resize-none shadow focus:outline-none focus:border-beigeD focus:ring-beigeD focus:ring-1 focus:shadow-lg">
                        {{ noteTitle }}
                    </textarea>
                </div>
                <div class="m-5 flex justify-between">
                    <textarea v-model="noteContent" wrap="hard" class="p-2 h-140 w-full text-sm text-black rounded resize-none shadow focus:outline-none focus:border-beigeD focus:ring-beigeD focus:ring-1 focus:shadow-lg">
                        {{ noteContent }}
                    </textarea>
                </div>
            </form>
