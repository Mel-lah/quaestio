...
<template>
    <div class="flex flex-col mx-3 mb-3">
        <div class="flex pt-3  items-center">     
            <textarea v-model="subjTitle"  class="p-2 mr-3 text-xl font-semibold italic text-black bg-beigeL rounded resize-none w-full h-10 overscroll-none overflow-hidden shadow ">{{ subjTitle }}</textarea>
        ...
        <div class="flex flex-col my-2">
            ...
            <textarea v-model="subjDscrptn" rows="4" wrap="hard" class="p-2 w-full text-sm text-black  rounded resize-none shadow">{{ subjDscrptn }}</textarea>
        </div> 
        <div class="inline-flex items-center ml-1 ">
            <h4 class="mr-2 text-lg font-bold"> Notes</h4>
            <button @click="storeCounter.showCNote" class="shadow hover:shadow-lg hover:bg-white hover:bg-opacity-80">
                <img class="w-3 h-3 hover:opacity-40" src="/img/add.png">
            </button>
        </div>
        <div class="flex flex-col overflow-y-scroll pt-3 h-128 mb-3 bg-white rounded shadow">
            <div v-for="nt in notes" class="mx-3">
                <div class="flex justify-center">
                    <button @click="[openNote(nt.id), storeCounter.showONote()]" class="h-10 w-full mb-3 bg-beigeD rounded-3xl shadow hover:bg-beigeSD" >
                        {{ nt.note_title }}
                    </button>
                    <button @click="deleteNote(nt.id)" class="justify-end h-10 ml-4 text-black font-bold hover:text-red-500 ">
                        &cross;
                    </button>    
                </div> 
            </div>
