...
<template>
    <div class="h-screen flex flex-col justify-center items-center">
        <div class="md:mr-100 mr-80">
            <button @click="storeCounter.showAnswerQuiz" class="rounded p-1 hover:bg-white hover:bg-opacity-80">
        ...
        <div class="bg-beigeL rounded-lg shadow-2xl w-96 md:w-200 h-3/4">  
            ...
            <div class="flex flex-col  overflow-y-scroll h-128 bg-white mx-5 rounded p-5 shadow ">
                <div v-for="(question, index) in questions" class="flex  bg-gray-200 rounded mb-5">
                    <div class="grid content-between h-64 md:h-44">
                        <div class="mx-5 mt-2 mb-2 text-lg overflow-auto ">
                           {{ question.questionq}}
                        </div>
                        <input v-model="userAnswer[index]" type="text" class="p-3 mx-3 md:mx-auto md:ml-8 h-10 w-64 md:w-150 mb-2 rounded-md border-gray-300 border-1 shadow-sm focus:outline-none focus:border-beigeD focus:ring-beigeD focus:ring-1 focus:shadow-lg">
                    </div> 
                </div>
            </div>   
            <div class="flex justify-center mt-5 ">
                <button @click="[storeCounter.showReviewQuiz()]" class="text-lg font-semibold bg-beigeD h-10 w-52 rounded-lg shadow-md hover:bg-beigeSD">
                    Submit
                </button>
            </div>
        </div>
    </div>
    <div v-if="sReviewQuiz" 
